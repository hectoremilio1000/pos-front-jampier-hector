...
export default function PurchaseOrderFormDrawer({
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      supplierId: isEdit
        ? (order?.supplierId ?? order?.supplier?.id ?? null)
        : null,
      warehouseId: isEdit
        ? (order?.warehouseId ?? order?.warehouse?.id ?? undefined)
        : undefined,
  ...
  async function submit({ openItems }: { openItems: boolean }) {
    ...
    const headerPayload = {
      ...
      applicationDate: v.applicationDate
        ? dayjs(v.applicationDate).format("YYYY-MM-DD")
        : null,
  ...
  return (
    ...
    >
      ...
      <div style={{ opacity: 0.65, marginTop: 12, marginBottom: 12 }}>
        * Siguiente paso: agregar líneas (presentación + cantidad + precio) y
        luego “Recibir”.
      </div>
