...
export default function AreaModal({
  ...
  useEffect(() => {
    const fetchPrinters = async () => {
      ...
      try {
        const baseUrl = user?.restaurant?.localBaseUrl;
        if (!baseUrl) {
          setPrinters([]);
          return;
        }

        const res = await fetch(`${baseUrl}/nprint/printers`, {
          cache: "no-store",
        });
        ...
        if (!res.ok) {
          throw new Error(`printers_failed_${res.status}`);
        }
        ...
        const sharedPrinters = list.filter((p) => p.shared === "TRUE");
        setPrinters(sharedPrinters.length ? sharedPrinters : list);
      } catch (e) {
        console.error(e);
